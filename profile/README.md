# Community Access

**Making software accessible by default, not as an afterthought.**

Community Access is a community-driven open-source organization building AI-powered tools that enforce accessibility standards in developer workflows. Our agents work inside Claude Code, GitHub Copilot, and Claude Desktop so that every line of code ships inclusive.

## What We Build

- **[Accessibility Agents](https://github.com/Community-Access/a11y-agent-team)**: 35 specialized AI agents that enforce WCAG 2.2 AA compliance across web code, Office documents, PDFs, and Markdown. Install in one command.

## Why This Exists

AI coding tools generate inaccessible code by default. They forget ARIA rules, skip keyboard navigation, ignore contrast ratios, and produce modals that trap screen reader users. We built the agents we wished existed and opened them to the world.

## Get Involved

We are built by and for the blind and low vision community. Your lived experience, domain knowledge, and contributions make these tools better for everyone.

- [Contributing Guide](https://github.com/Community-Access/a11y-agent-team/blob/main/CONTRIBUTING.md)
- [Code of Conduct](https://github.com/Community-Access/a11y-agent-team/blob/main/CODE_OF_CONDUCT.md)
- [Open Issues](https://github.com/Community-Access/a11y-agent-team/issues)

## Owners

- [Jeff Bishop](https://github.com/jeffreybishop)
- [Taylor Arndt](https://github.com/taylorarndt)
